...
func (n *node) addRoute(path string, handler *event) {
	fullPath := path
	n.priority++
...
func (n *node) insertChild(numParams uint8, path, fullPath string, handler *event) {
...
// given path.
func (n *node) getValue(path string) (handler *event, p Params, tsr bool) {
